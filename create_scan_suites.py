...
def run_mutation(endpoint: str, token: str, mutation: str):
    headers = {
...
def main():
    config = load_config("scan_config.json")
    endpoint = config["graphqlEndpoint"]
    token = config["token"]
    owner_ids = [oid.strip() for oid in config["ownerValue"].split(",")]

    for owner_id in owner_ids:
        print(f"Creating scan suite for Owner ID: {owner_id}")
        mutation = build_mutation(config, owner_id)
        result = run_mutation(endpoint, token, mutation)
